...
import { isArchiveMetadataPath } from "../../file-utils.js";
  ...
  while (offset + BLOCK_SIZE <= bytes.length) {
    ...
    if (entryCount > MAX_ODB_ARCHIVE_ENTRY_COUNT) {
      throw new RangeError(
        `${archiveName} contains more than ${MAX_ODB_ARCHIVE_ENTRY_COUNT} TAR entries`,
      );
    }
...
function readPaxHeaders(data, archiveName, entryCount) {
  ...
  while (position < text.length) {
    const spaceIndex = text.indexOf(" ", position);
    if (spaceIndex === -1) break;
    const length = Number.parseInt(text.slice(position, spaceIndex), 10);
    if (!Number.isSafeInteger(length) || length <= 0) {
      throw new Error(`${archiveName} TAR entry ${entryCount} has an invalid PAX record`);
    }
    const record = text.slice(spaceIndex + 1, position + length - 1);
