...
- [BGA 测试图案](https://wasm-gerber-viewer.vercel.app/?url=https%3A%2F%2Fwasm-gerber-viewer.vercel.app%2Fdemo%2Fbga-test-patterns.gbr)
...
- 支持 NC drill 叠加渲染
- 支持导入 ODB++ 作业（`.tgz`/`.tar.gz`/`.tar`、包含 ODB++ 目录树的 `.zip`、拖放文件夹；作业内的 `.Z`/`.gz` 压缩文件）- 在 WASM 中将板层、钻孔和 step profile 直接解析为渲染几何，并展开用户自定义符号
- 支持移动设备触控操作
- 支持按层控制颜色、透明度和可见性
